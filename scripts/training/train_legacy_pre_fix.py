...
kwargs = InitProcessGroupKwargs(backend="nccl", timeout=timedelta(minutes=180))
...
if accelerator.is_main_process:
    ...
    is_tied = (model.encoder.embed_tokens.weight is model.shared.weight)
    print(f"Weight Tying Status: {is_tied} (Must be True!)")
...
with accelerator.main_process_first():
    ...
    eval_raw = load_dataset("json", data_files={"eval": "EvalDataModel/*.jsonl"}, streaming=True)["eval"]
...
if last_epoch is not None and last_epoch > START_EPOCH:
    try:
        accelerator.load_state(f"{SNAPSHOT_DIR}/model_epoch{last_epoch}")
        if accelerator.is_main_process: print(f"DeepSpeed resume from epoch {last_epoch}")
    except: pass
else:
...
for epoch in range(last_epoch + 1, num_epochs_total + 1):
    train_epoch = train_tok.shuffle(seed=epoch, buffer_size=50000)
    train_loader = DataLoader(train_epoch, batch_size=batch_size, drop_last=True, collate_fn=collator, num_workers=4)
    train_loader = accelerator.prepare(train_loader)

    model.train()
    total_loss, batches = 0.0, 0
    if accelerator.is_main_process: print(f"\n=== Epoch {epoch} Start (Clean Start Mode) ===")
    ...
    for step, batch in enumerate(progress_bar, start=1):
        ...
        batches += 1
        if accelerator.is_main_process: progress_bar.set_description(f"Loss: {loss.item():.4f}")
    ...
    save_path = f"{SNAPSHOT_DIR}/model_epoch{epoch}"
    ...
    if accelerator.is_main_process:
        print(f"Epoch {epoch} beendet. Train Loss: {avg_train:.4f}, Eval Loss: {avg_eval:.4f}")