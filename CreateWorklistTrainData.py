...
def main():
    ...
    worklist_path = os.path.join(os.path.dirname(__file__), "worklist.tsv")

    work_items: list[str] = []

    for _, exec_path, functions in collect_executable_function_metadata():
        if not functions:
            continue
        ...
        # HIER: statt subprocess pro Funktion => wir sammeln
        for func in functions:
            # Format: absolute_binary_path<TAB>function_name
            work_items.append(f"{os.path.abspath(binary_path)}\t{func}")

    if not work_items:
        print("[INFO] no work items found. nothing to do.")
        return
