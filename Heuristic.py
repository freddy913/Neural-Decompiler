...
def _is_trampoline_or_tiny(func_obj, max_blocks=2, max_insns=8):
    ...
    if len(blocks) > max_blocks: return False
...
def is_relevant_user_like_function(func_obj, target_func_obj=None):
    ...
    if name in RUNTIME_ENTRY_FUNCTIONS: return False
    if getattr(func_obj, "is_plt", False): return False
    if name.startswith("__"): return False
    if _is_trampoline_or_tiny(func_obj): return False
...
HEADER_MAP = {
    "<stdio.h>": ["printf", "fprintf", "sprintf", "snprintf", "puts", "putchar", "perror", "scanf", "fscanf", "sscanf", "fgets", "fputs", "fopen", "fclose", "fread", "fwrite"],
    "<stdlib.h>": ["malloc", "calloc", "realloc", "free", "exit", "abort", "atoi", "atof", "strtol", "strtoul", "qsort", "rand", "srand", "abs", "labs"],
    "<string.h>": ["strcpy", "strncpy", "strcat", "strncat", "strcmp", "strncmp", "strlen", "strstr", "memcpy", "memmove", "memcmp", "memset", "strerror", "strtok", "strpbrk", "strrchr"],
    "<math.h>": ["sin", "cos", "tan", "sqrt", "pow", "log", "exp", "log10", "ceil", "floor"],
    "<unistd.h>": ["read", "write", "close", "sleep", "usleep", "fork", "execve", "pipe", "dup", "dup2", "getpid"],
    "<pthread.h>": ["pthread_create", "pthread_join", "pthread_mutex_lock", "pthread_mutex_unlock", "pthread_cond_wait", "pthread_cond_signal", "pthread_rwlock_init", "pthread_rwlock_destroy", "pthread_rwlock_rdlock", "pthread_rwlock_wrlock"],
    "<netinet/in.h>": ["socket", "bind", "listen", "accept", "inet_addr", "htons", "htonl", "ntohs", "ntohl"],
    "<sys/types.h>": ["open", "close", "read", "write", "lseek", "stat", "fstat", "unlink", "rename"],
    "<time.h>": ["clock", "time", "ctime", "difftime", "strftime", "localtime", "mktime", "gmtime", "asctime"],
    "<fcntl.h>": ["open", "close", "read", "write", "fcntl", "ioctl", "pipe2", "dup2", "select"],
}

def _strip_symbol_version(sym: str) -> str:
    if '@@' in sym: return sym.split('@@')[0]
    elif '@' in sym: return sym.split('@')[0]
    return sym

def _readelf_symbols(binary_path: str) -> list[str]:
    if shutil.which("readelf") is None: return []
    try:
        out = subprocess.check_output(["readelf", "-s", binary_path], text=True, errors="ignore")
    except Exception: return []
    
    symbols = []
    for line in out.splitlines():
        if "UND" not in line and "UND " not in line and "UNDEF" not in line: continue
        parts = line.strip().split()
        if not parts: continue
        cand = parts[-1]
        if cand in ("UND", "UNDEF"): continue
        symbols.append(cand)
    return symbols
    ...
    for header, sym_list in HEADER_MAP.items():
        for s in symbols:
            if s in sym_list:
                ...
                break
    return list(dict.fromkeys(needed_headers))
...
def build_header_block_from_binary(binary_path: str) -> str:
    ...
    if not headers: block = ""
    else:
        lines = ["HEADERS:"]
        for h in headers: lines.append(f"#include {h}")
        block = "\n".join(lines) + "\n"
...
# Scoring Features and Helpers
def _candidate_func_calls_target(candidate_addr, cg, target_addr):
    try: return target_addr in cg.successors(candidate_addr)
    except Exception: return False

def _target_calls_candidate(target_addr, cg, candidate_addr):
    try: return candidate_addr in cg.successors(target_addr)
    except Exception: return False

def _get_block_and_insn_counts(func_obj):
    if func_obj is None: return None, None
    try: blocks = list(func_obj.blocks)
...
def _is_stub_like(func_obj):
    ...
    if name in JUNK_FUNCTIONS or name.startswith("__"): return True
    if getattr(func_obj, "is_plt", False): return True
    return _is_trampoline_or_tiny(func_obj)

def _cfg_fingerprint(func_obj):
    if func_obj is None: return None
    try: g = func_obj.graph
    ...
    return {"nodes": nodes, "edges": edges, "cyclomatic": cyclomatic, "branch_ratio": branch_ratio}
...
def _cfg_similarity_bonus(target_obj, candidate_obj, strong_threshold=0.7):
    ...
    if not tgt_fp or not cand_fp: return 0
    ...
    if similarity >= strong_threshold: return 10
    return 0
...
def _split_identifiers(name: str):
    ...
    for p in parts:
        ...
        if subtokens: tokens.extend(subtokens)
...
def _struct_overlap_bonus(target_struct_fp, candidate_struct_fp):
    ...
    bonus = 0
    if specific: bonus += 25 + max(len(specific) - 1, 0) * 5
    if common: bonus += min(5, len(common))
    union_size = len(target_struct_fp | candidate_struct_fp)
    coverage = len(overlap) / max(union_size, 1)
    if len(overlap) >= 2 and coverage >= 0.5: bonus += 5
    return bonus
...
_REGEX_REG = re.compile(r"\b(?:r[0-9]+|r1[0-5]|r[8-9]|rax|rbx|rcx|rdx|rsi|rdi|rsp|rbp|eax|ebx|ecx|edx|esi|edi|esp|ebp|al|ah|bl|bh|cl|ch|dl|dh)\b", re.IGNORECASE)
...
def _callgraph_distance_bonus(degree, has_data_overlap):
    if degree == 2 and has_data_overlap: return 5
    return 0

def _is_small_leaf_wrapper(candidate_obj, cg, target_addr, max_blocks=30, max_insns=50):
    if candidate_obj is None: return False
    try: succ_addrs = set(cg.successors(candidate_obj.addr))
    except Exception: succ_addrs = set()
    non_target_calls = {addr for addr in succ_addrs if addr != target_addr}
    if non_target_calls: return False
...
def _calculate_candidate_score(candidate, callgraph, all_program_funcs, target_addr, target_struct_fp=None, target_src_loc=None, target_lex_fp=None, target_api_sig=None):
    ...
    if candidate_addr is None: return candidate["score"]
...
def add_candidate_to_context(remaining_candidates, context_funcs, candidate, current_budget):
    """
    Adds a candidate (Assembly mode) to the context if budget allows.
    """
...
def apply_heuristic(
    ...
    for candidate in scored_candidates:
        ...
        current_budget, added = add_candidate_to_context(
            [],
            context_funcs, 
            candidate, 
            current_budget
...
# DWARF / Source Helpers to extract ground truth C-Code for training.
def _decode_dwarf_str(value):
    if isinstance(value, bytes): return value.decode("utf-8", "ignore")
    return value

def _extract_function_from_source(src_text, func_name):
    if not src_text or not func_name: return None
    if not re.search(rf'\b{re.escape(func_name)}\s*\(', src_text): return None

    sig_regex = re.compile(rf'([A-Za-z0-9_\*\s]+?\b{re.escape(func_name)}\s*\([^;]*\)\s*\{{)', re.MULTILINE)
    ...
    while i < n:
        ...
        if in_string:
            ...
            if ch in ('"', "'"):
                ...
                brace_depth -= 1
                if brace_depth == 0:
                    return src_text[start_idx:i+1].lstrip()
        i += 1
    return None
...
def _scan_dir_for_func_once(dir_path, func_name, exact_filename=None):
    if not dir_path or not os.path.isdir(dir_path): return None
    if exact_filename:
...
def _resolve_source_path_from_die(candidate, binary_path=None):
    die = candidate.get("die")
    cu = candidate.get("cu")
    dwarf_info = candidate.get("dwarf_info")
    if not die or not cu or not dwarf_info: return None

    try: line_program = dwarf_info.line_program_for_CU(cu)
    except Exception: return None
    if not line_program: return None

    file_attr = die.attributes.get("DW_AT_decl_file")
    if not file_attr: return None
    try: file_entry = line_program["file_entry"][file_attr.value - 1]
    except Exception: return None
    ...
    try: include_dirs = line_program["include_directory"]
    except Exception: include_dirs = []
    if dir_index and dir_index - 1 < len(include_dirs):
        dir_entry = _decode_dwarf_str(include_dirs[dir_index - 1])
        if dir_entry: dir_candidates.append(dir_entry)
    ...
    if comp_dir: dir_candidates.append(comp_dir)
    ...
    if os.path.isabs(file_name):
        ...
        for base in dir_candidates:
            ...
            potential_paths.append(os.path.normpath(os.path.join(base, file_name)))
        potential_paths.append(os.path.normpath(file_name))
...
def _extract_source_text_from_die(candidate, binary_path=None):
    ...
    func_name_attr = candidate.get("die").attributes.get("DW_AT_name")
    ...
    text = _load_file_text(source_path)
...
def _die_addr_range(candidate):
    ...
    low_pc = low.value
    if not high: return (low_pc, low_pc)
    hv = high.value
    if isinstance(hv, int) and hv < 0x100000: high_pc = low_pc + hv
    else: high_pc = hv
...
def get_real_c_code(
    ...
    *,
    ...
    if source_hint:
        snippet = _load_file_text(source_hint)
        if snippet:
            res = _extract_function_from_source(snippet, func_name)
            if res: return res

    binary_path = getattr(project, "filename", None)
    if binary_path is None:
        try: binary_path = project.loader.main_object.binary
        except Exception: binary_path = None
...
"""
FUTURE WORK / EXPERIMENTAL C-CODE CONTEXT
These functions are preserved for future experiments involving on-the-fly 
decompilation or C-based context strategies. They are NOT used in the current pipeline.
"""
...
def add_candidate_as_c_code_to_context(remaining_candidates, context_funcs, candidate, current_budget, mode):
    ...
    def _only_add_real_c_code():
        ...
        context_funcs.append(candidate)
        try: remaining_candidates.remove(candidate)
        except ValueError: pass
        # Note: current_budget is int (immutable), so modifying it inside closure doesn't affect outer scope unless returned
        # Logic here was complex and is deprecated.
        pass

    # Deprecated Logic Placeholder
...
def add_decompiled_candidate_to_context(remaining_candidates, context_funcs, candidate, current_budget): 
    token_count = get_token_count(candidate.get('c_approx', ''))
...
def token_degree_level_check(remaining_candidates):
    if not remaining_candidates: return None, 0
...
def add_remaining_candidates_to_context(remaining_candidates, context_funcs, current_budget, REDUCTION_LEVEL):
    # Deprecated loop for mixed C/Asm filling
    return current_budget, remaining_candidates

def estimate_c_token_complexity(func):
    if not func: return float('inf')
    function_cfg = func.graph
    if not function_cfg: return 1
